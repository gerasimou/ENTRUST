...
RQVMOOS::RQVMOOS() : FIXED_DISTANCE(10), MIN_SPEED(0.1), ARRAY_SIZE(147), CSLPROPPERTIES(2),
    ...
    m_previous_iterate_timestamp = MOOSTime(true) - 10;//GetAppStartTime();
...
//			m_previous_y = m_current_y;
            ...
            vector<string> svector = parseString(msg.GetString(),
                                                 ","); // svector[0] = "name=sonar", svector[1] = "reading_rate=5"
...
void RQVMOOS::sendNotifications(double looptime) {
    ...
    sensorsRates =
            doubleToString(m_sensor_map.find("SONAR_SENSOR_SONAR1")->second.readingRateTimes / timeStamp) + " - " +
    ...
    Notify("VIEW_MARKER",
           "type=circle,x=25,y=-65,scale=2,msg=r1:" + doubleToString(sensor1.readingRateAvg).substr(0, 5) +
           ",label=r1,color=darkblue,width=1");
    ...
    Notify("VIEW_MARKER",
           "type=circle,x=75,y=-65,scale=2,msg=r2:" + doubleToString(sensor2.readingRateAvg).substr(0, 5) +
           ",label=r2,color=darkblue,width=1");
    ...
    Notify("VIEW_MARKER",
           "type=circle,x=125,y=-65,scale=2,msg=r3:" + doubleToString(sensor3.readingRateAvg).substr(0, 5) +
           ",label=r3,color=darkblue,width=1");


    //speed
    Notify("UPDATES_BHV_CONSTANT_SPEED", "speed=" + doubleToString(m_desired_uuv_speed));
    Notify("VIEW_MARKER",
           "type=circle,x=75,y=-80,scale=2,msg=Speed:" + doubleToString(m_desired_uuv_speed).substr(0, 4) +
           ",label=speed,color=darkblue,width=1");
...
bool RQVMOOS::buildReport() {
    ...
    m_msgs << "S1)\t" << sensor1.readingRate << "\t" << "\t" << sensor1.readingRateAvg << "\t" <<
    (sensor1.readingRateAvg < 5 * 0.75 ? "Down!" : "") << "\t" << sensor1.currentState << "\t" << sensor1.newState <<
    "\t" << sensor1.sensorIdleTimes << endl;
    m_msgs << "S2)\t" << sensor2.readingRate << "\t" << "\t" << sensor2.readingRateAvg << "\t" <<
    (sensor2.readingRateAvg < 4 * 0.75 ? "Down!" : "") << "\t" << sensor2.currentState << "\t" << sensor2.newState <<
    "\t" << sensor2.sensorIdleTimes << endl;
    m_msgs << "S3)\t" << sensor3.readingRate << "\t" << "\t" << sensor3.readingRateAvg << "\t" <<
    (sensor3.readingRateAvg < 4 * 0.75 ? "Down!" : "") << "\t" << sensor3.currentState << "\t" << sensor3.newState <<
    "\t" << sensor3.sensorIdleTimes << endl;
...
bool RQVMOOS::invokeManagingSystem() {
    ...
    double sensor1AvgReadingRate = estimateReadingRate("SONAR_SENSOR_SONAR1", 5, 2);
    double sensor2AvgReadingRate = estimateReadingRate("SONAR_SENSOR_SONAR2", 4, 4);
    double sensor3AvgReadingRate = estimateReadingRate("SONAR_SENSOR_SONAR3", 4, 8);
...
double RQVMOOS::estimateReadingRate(string sensorName, double sensorNormalOperatingRate,
                                    int sensorConfigurationActive) {
