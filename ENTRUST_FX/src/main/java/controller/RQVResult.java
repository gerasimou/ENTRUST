...
public class RQVResult{
	...
	public RQVResult (int sConfig[], double req1Result, double req2Result, double req3Result){
		...
		this.req1Result	= (int) (ENTRUST.MULTIPLIER_RELIABILITY * req1Result); //Math.round(req1Result);
		this.req2Result	= (int) (ENTRUST.MULTIPLIER * req2Result);//Math.round(req2Result);
		this.req3Result = (int) (ENTRUST.MULTIPLIER * req3Result);//Math.round(req3Result);
	}
